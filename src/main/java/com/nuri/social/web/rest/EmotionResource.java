...
/**
 * REST controller for managing {@link com.nuri.social.domain.Emotion}.
 */
@RestController
@RequestMapping("/api")
public class EmotionResource {
	 ...
	 *         body the new emotionDTO, or with status {@code 400 (Bad Request)} if
	 *         the emotion has already an ID.
	 ...
	 *         the updated emotionDTO, or with status {@code 400 (Bad Request)} if
	 *         the emotionDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the emotionDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping("/emotions")
	public ResponseEntity<EmotionDTO> updateEmotion(@Valid @RequestBody EmotionDTO emotionDTO) throws URISyntaxException {
		...
		EmotionDTO result = emotionService.save(emotionDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, emotionDTO.getId())).body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the emotionDTO, or with status {@code 404 (Not Found)}.
	 */
