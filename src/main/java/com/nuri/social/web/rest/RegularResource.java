...
/**
 * REST controller for managing {@link com.nuri.social.domain.Regular}.
 */
@RestController
@RequestMapping("/api")
public class RegularResource {
	 ...
	 *         body the new regularDTO, or with status {@code 400 (Bad Request)} if
	 *         the regular has already an ID.
	 ...
	 *         the updated regularDTO, or with status {@code 400 (Bad Request)} if
	 *         the regularDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the regularDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping("/regulars")
	public ResponseEntity<RegularDTO> updateRegular(@Valid @RequestBody RegularDTO regularDTO) throws URISyntaxException {
		...
		RegularDTO result = regularService.save(regularDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, regularDTO.getId())).body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the regularDTO, or with status {@code 404 (Not Found)}.
	 */
	@GetMapping("/regulars/{id}")
	public ResponseEntity<RegularDTO> getRegular(@PathVariable String id) {
	...
	public ResponseEntity<Void> deleteRegular(@PathVariable String id) {
