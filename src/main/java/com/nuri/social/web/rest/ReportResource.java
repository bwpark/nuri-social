...
import com.nuri.social.service.ReportService;
...
/**
 * REST controller for managing {@link com.nuri.social.domain.Report}.
 */
@RestController
@RequestMapping("/api")
public class ReportResource {
	 ...
	 *         body the new reportDTO, or with status {@code 400 (Bad Request)} if
	 *         the report has already an ID.
	 ...
	 *         the updated reportDTO, or with status {@code 400 (Bad Request)} if
	 *         the reportDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the reportDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping("/reports")
	public ResponseEntity<ReportDTO> updateReport(@Valid @RequestBody ReportDTO reportDTO) throws URISyntaxException {
		...
		ReportDTO result = reportService.save(reportDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, reportDTO.getId())).body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the reportDTO, or with status {@code 404 (Not Found)}.
	 */
