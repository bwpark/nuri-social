...
/**
 * REST controller for managing {@link com.nuri.social.domain.Category}.
 */
@RestController
@RequestMapping("/api")
public class CategoryResource {
	 ...
	 *         body the new categoryDTO, or with status {@code 400 (Bad Request)} if
	 ...
	 *         the updated categoryDTO, or with status {@code 400 (Bad Request)} if
	 *         the categoryDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the categoryDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	...
	public ResponseEntity<CategoryDTO> updateCategory(@Valid @RequestBody CategoryDTO categoryDTO) throws URISyntaxException {
		...
		CategoryDTO result = categoryService.save(categoryDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, categoryDTO.getId())).body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the categoryDTO, or with status {@code 404 (Not Found)}.
	 */
