...
import com.nuri.social.service.IssueResourceService;
...
/**
 * REST controller for managing {@link com.nuri.social.domain.IssueResource}.
 */
@RestController
@RequestMapping("/api")
public class IssueResourceResource {
	 ...
	 *         body the new issueResourceDTO, or with status
	 *         {@code 400 (Bad Request)} if the issueResource has already an ID.
	 ...
	 *         the updated issueResourceDTO, or with status
	 *         {@code 400 (Bad Request)} if the issueResourceDTO is not valid, or
	 *         with status {@code 500 (Internal Server Error)} if the
	 *         issueResourceDTO couldn't be updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	...
	public ResponseEntity<IssueResourceDTO> updateIssueResource(@Valid @RequestBody IssueResourceDTO issueResourceDTO) throws URISyntaxException {
		...
		IssueResourceDTO result = issueResourceService.save(issueResourceDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, issueResourceDTO.getId())).body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the issueResourceDTO, or with status {@code 404 (Not Found)}.
	 */
	@GetMapping("/issue-resources/{id}")
	public ResponseEntity<IssueResourceDTO> getIssueResource(@PathVariable String id) {
	...
	public ResponseEntity<Void> deleteIssueResource(@PathVariable String id) {
