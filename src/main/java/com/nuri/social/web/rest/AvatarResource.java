...
import com.nuri.social.service.AvatarService;
...
/**
 * REST controller for managing {@link com.nuri.social.domain.Avatar}.
 */
@RestController
@RequestMapping("/api")
public class AvatarResource {
	 ...
	 *         body the new avatarDTO, or with status {@code 400 (Bad Request)} if
	 *         the avatar has already an ID.
	 ...
	 *         the updated avatarDTO, or with status {@code 400 (Bad Request)} if
	 *         the avatarDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the avatarDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping("/avatars")
	public ResponseEntity<AvatarDTO> updateAvatar(@Valid @RequestBody AvatarDTO avatarDTO) throws URISyntaxException {
		...
		AvatarDTO result = avatarService.save(avatarDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, avatarDTO.getId())).body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the avatarDTO, or with status {@code 404 (Not Found)}.
	 */
