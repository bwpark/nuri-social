...
/**
 * REST controller for managing {@link com.nuri.social.domain.Interact}.
 */
@RestController
@RequestMapping("/api")
public class InteractResource {
	 ...
	 *         body the new interactDTO, or with status {@code 400 (Bad Request)} if
	 *         the interact has already an ID.
	 ...
	 *         the updated interactDTO, or with status {@code 400 (Bad Request)} if
	 *         the interactDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the interactDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	...
	public ResponseEntity<InteractDTO> updateInteract(@Valid @RequestBody InteractDTO interactDTO) throws URISyntaxException {
		...
		InteractDTO result = interactService.save(interactDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, interactDTO.getId())).body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the interactDTO, or with status {@code 404 (Not Found)}.
	 */
	@GetMapping("/interacts/{id}")
	public ResponseEntity<InteractDTO> getInteract(@PathVariable String id) {
	...
	public ResponseEntity<Void> deleteInteract(@PathVariable String id) {
