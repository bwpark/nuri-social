...
/**
 * REST controller for managing {@link com.nuri.social.domain.Issue}.
 */
@RestController
@RequestMapping("/api")
public class IssueResource {
	 ...
	 *         body the new issueDTO, or with status {@code 400 (Bad Request)} if
	 ...
	 *         the updated issueDTO, or with status {@code 400 (Bad Request)} if the
	 *         issueDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the issueDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping("/issues")
	public ResponseEntity<IssueDTO> updateIssue(@Valid @RequestBody IssueDTO issueDTO) throws URISyntaxException {
		...
		IssueDTO result = issueService.save(issueDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, issueDTO.getId())).body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the issueDTO, or with status {@code 404 (Not Found)}.
	 */
