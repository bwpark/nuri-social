...
/**
 * REST controller for managing {@link com.nuri.social.domain.Repute}.
 */
@RestController
@RequestMapping("/api")
public class ReputeResource {
	 ...
	 *         body the new reputeDTO, or with status {@code 400 (Bad Request)} if
	 *         the repute has already an ID.
	 ...
	 *         the updated reputeDTO, or with status {@code 400 (Bad Request)} if
	 *         the reputeDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the reputeDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping("/reputes")
	public ResponseEntity<ReputeDTO> updateRepute(@Valid @RequestBody ReputeDTO reputeDTO) throws URISyntaxException {
		...
		ReputeDTO result = reputeService.save(reputeDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, reputeDTO.getId())).body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the reputeDTO, or with status {@code 404 (Not Found)}.
	 */
	@GetMapping("/reputes/{id}")
	public ResponseEntity<ReputeDTO> getRepute(@PathVariable String id) {
	...
	public ResponseEntity<Void> deleteRepute(@PathVariable String id) {
