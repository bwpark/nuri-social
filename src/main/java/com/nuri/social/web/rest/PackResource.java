...
import com.nuri.social.service.PackService;
...
/**
 * REST controller for managing {@link com.nuri.social.domain.Pack}.
 */
@RestController
@RequestMapping("/api")
public class PackResource {
	 ...
	 *         body the new packDTO, or with status {@code 400 (Bad Request)} if the
	 *         pack has already an ID.
	 ...
	 *         the updated packDTO, or with status {@code 400 (Bad Request)} if the
	 *         packDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the packDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping("/packs")
	public ResponseEntity<PackDTO> updatePack(@Valid @RequestBody PackDTO packDTO) throws URISyntaxException {
		...
		PackDTO result = packService.save(packDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, packDTO.getId())).body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the packDTO, or with status {@code 404 (Not Found)}.
	 */
	@GetMapping("/packs/{id}")
	public ResponseEntity<PackDTO> getPack(@PathVariable String id) {
