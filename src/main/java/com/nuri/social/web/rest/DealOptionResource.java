...
/**
 * REST controller for managing {@link com.nuri.social.domain.DealOption}.
 */
@RestController
@RequestMapping("/api")
public class DealOptionResource {
	 ...
	 *         body the new dealOptionDTO, or with status {@code 400 (Bad Request)}
	 *         if the dealOption has already an ID.
	 ...
	 *         the updated dealOptionDTO, or with status {@code 400 (Bad Request)}
	 *         if the dealOptionDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the dealOptionDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	...
	public ResponseEntity<DealOptionDTO> updateDealOption(@Valid @RequestBody DealOptionDTO dealOptionDTO) throws URISyntaxException {
		...
		DealOptionDTO result = dealOptionService.save(dealOptionDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, dealOptionDTO.getId())).body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the dealOptionDTO, or with status {@code 404 (Not Found)}.
	 */
	@GetMapping("/deal-options/{id}")
	public ResponseEntity<DealOptionDTO> getDealOption(@PathVariable String id) {
	...
	public ResponseEntity<Void> deleteDealOption(@PathVariable String id) {
