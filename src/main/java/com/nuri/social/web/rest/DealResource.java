...
import com.nuri.social.service.DealService;
...
/**
 * REST controller for managing {@link com.nuri.social.domain.Deal}.
 */
@RestController
@RequestMapping("/api")
public class DealResource {
	 ...
	 *         body the new dealDTO, or with status {@code 400 (Bad Request)} if the
	 ...
	 *         the updated dealDTO, or with status {@code 400 (Bad Request)} if the
	 *         dealDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the dealDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping("/deals")
	public ResponseEntity<DealDTO> updateDeal(@Valid @RequestBody DealDTO dealDTO) throws URISyntaxException {
		...
		DealDTO result = dealService.save(dealDTO);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, dealDTO.getId())).body(result);
	}
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the dealDTO, or with status {@code 404 (Not Found)}.
	 */
	@GetMapping("/deals/{id}")
