...
public class AvatarCategoryService {
	...
	public List<AvatarCategoryDTO> findAll() {
		...
		return avatarCategoryRepository.findAll().stream().map(avatarCategoryMapper::toDto).collect(Collectors.toCollection(LinkedList::new));
	}
	...
	public Optional<AvatarCategoryDTO> findOne(String id) {
		...
		return avatarCategoryRepository.findById(id).map(avatarCategoryMapper::toDto);
	}
