...
public class TOSService {
	...
	public List<TOSDTO> findAll() {
		...
		return tOSRepository.findAll().stream().map(tOSMapper::toDto).collect(Collectors.toCollection(LinkedList::new));
	}
	...
	public Optional<TOSDTO> findOne(String id) {
		...
		return tOSRepository.findById(id).map(tOSMapper::toDto);
	}
