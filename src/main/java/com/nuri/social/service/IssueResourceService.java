...
public class IssueResourceService {
	...
	public List<IssueResourceDTO> findAll() {
		...
		return issueResourceRepository.findAll().stream().map(issueResourceMapper::toDto).collect(Collectors.toCollection(LinkedList::new));
	}
	...
	public Optional<IssueResourceDTO> findOne(String id) {
		...
		return issueResourceRepository.findById(id).map(issueResourceMapper::toDto);
	}
