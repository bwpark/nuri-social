...
public class IssueOptionService {
	...
	public List<IssueOptionDTO> findAll() {
		...
		return issueOptionRepository.findAll().stream().map(issueOptionMapper::toDto).collect(Collectors.toCollection(LinkedList::new));
	}
	...
	public Optional<IssueOptionDTO> findOne(String id) {
		...
		return issueOptionRepository.findById(id).map(issueOptionMapper::toDto);
	}
