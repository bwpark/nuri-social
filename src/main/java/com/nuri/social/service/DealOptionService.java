...
public class DealOptionService {
	...
	public List<DealOptionDTO> findAll() {
		...
		return dealOptionRepository.findAll().stream().map(dealOptionMapper::toDto).collect(Collectors.toCollection(LinkedList::new));
	}
	...
	public Optional<DealOptionDTO> findOne(String id) {
		...
		return dealOptionRepository.findById(id).map(dealOptionMapper::toDto);
	}
