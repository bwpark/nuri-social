...
public class ChemistryService {
	...
	public List<ChemistryDTO> findAll() {
		...
		return chemistryRepository.findAll().stream().map(chemistryMapper::toDto).collect(Collectors.toCollection(LinkedList::new));
	}
	...
	public Optional<ChemistryDTO> findOne(String id) {
		...
		return chemistryRepository.findById(id).map(chemistryMapper::toDto);
	}
